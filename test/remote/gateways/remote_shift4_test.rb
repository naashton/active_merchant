...
class RemoteShift4Test < Test::Unit::TestCase
  ...
  def test_successful_capture
    ...
    auth_token = authorize_res.authorization
  ...
  def successful_purchase_and_refund
    purchase = @gateway.purchase(@amount, @token, @purchase_params)
    assert_success purchase

    transaction = purchase.params['result'][0]['transaction']
    card = purchase.params['result'][0]['card']
    invoice = transaction['invoice']
    auth_token = card['token']['value']
    @refund_options[:invoice] = invoice

    response = @gateway.refund(@amount, auth_token, @refund_options)
  ...
  def test_successful_void
    authorize_res = @gateway.authorize(@amount, @credit_card, @options)
    invoice = authorize_res.params['result'][0]['transaction']['invoice']
    @void_options[:invoice] = invoice
    assert response = @gateway.void(@void_options)
